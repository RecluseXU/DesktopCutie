...
from infoTool.load_Project_Location import get_ProjectLocation,\
    get_judge_model_folder_location
...
class Talk_set_tab(QWidget, Ui_talk_set_tab):
    ...
    def __init__(self, parent=None):
        ...
        self.add_pushButton.clicked.connect(self.click_add_judgement_button)
        self.delete_pushButton.clicked.connect(self.click_delete_judgement_button)
        self.open_judgement_folder_button.clicked.connect(self.click_open_judgement_folder_button)
        
        print("精灵设置页加载完毕")
    
    def click_add_judgement_button(self):
        '''
        点击添加按钮
        '''
        file_location, _ = QFileDialog.getOpenFileName(self, "OpenFile", get_ProjectLocation(), "python source file(*.py)")
        if(len(file_location)>0):
            file_name = file_location.split("/")[-1]
            aim_location = get_judge_model_folder_location()+file_name
            talk_set_console.copy_file_to_judgement_folder(file_location, aim_location)
            self.loading_judgement()
        
    def click_delete_judgement_button(self):
        '''
        点击删除模组按钮
        '''
        current_judge_name = self.judge_treeWidget.currentItem().text(0)
        talk_set_console.delete_a_selected_judgement(current_judge_name)
        self.loading_judgement()
